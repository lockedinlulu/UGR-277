...
- **AI Core (Component 2):** n8n detects new incident reports, sends them to Flowise via HTTP request, Flowise classifies the incident using 3 chained LLM prompts (classify → analyze → recommend), n8n writes structured results back to Airtable
...
- **Integration (Component 4):** Dashboard connecting all components — NOT YET BUILT as of Checkpoint 2

## Tech Stack
- n8n Cloud (workflow automation)
- Flowise Cloud (LLM chains — 3 chains on single chatflow due to free tier limits)
...
### Emergency_Protocols (Component 1) — EXISTS
...
### Incident_Reports (Component 2 input) — NOT YET IN SHARED BASE
| Field | Type | Description |
|-------|------|-------------|
| incident_text | Long Text | Raw free-text incident report |
| status | Single Select | new / classified / error |

### Incidents (Component 2 output) — NOT YET IN SHARED BASE
| Field | Type | Description |
|-------|------|-------------|
...
### Protocol_Steps (Component 3) — NOT YET IN SHARED BASE
| Field | Type | Description |
|-------|------|-------------|
...
### Tasks (Component 3 output) — NOT YET IN SHARED BASE
| Field | Type | Description |
|-------|------|-------------|
...
## Conventions
- Field names: snake_case preferred (Tasks table has inconsistent casing — known issue)
...
- **Working:** Component 1 fully built (5 protocols in Airtable). Component 2 classifying all 3 emergency types correctly via Flowise + n8n. Component 3 generating tasks with role assignments.
- **Not working:** Component 4 dashboard not built. Incidents and Tasks tables not in shared Airtable base. No true end-to-end flow verified.
- **Known issues:** Field naming inconsistency (protocol_id vs Protocol_Id). Shared Airtable base only has Protocols table.
- **Next milestone:** Build Component 4, create shared Incidents/Tasks tables, run true end-to-end test.
